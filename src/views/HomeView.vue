...
export default {
  ...
  async created() {
    ...
    d3.camera = new THREE.PerspectiveCamera(75, rect.width / rect.height, 0.1, 1000)
    ...
    d3.contorls = new OrbitControls(d3.camera, this.$refs.canvas)
    d3.contorls.target.set(0, 0, 0)

    d3.lights.ambient = new THREE.AmbientLight(0xFFFFFF, 1.5)
    d3.scene.add(d3.lights.ambient)
    d3.lights.hemi = new THREE.HemisphereLight(0xddaaff, 0x564345, 1.5)
    d3.scene.add(d3.lights.hemi)
    d3.lights.dir = new THREE.DirectionalLight(0xFFDDDD, 15)
    d3.lights.dir.position.set(0, 100, 0)
    d3.scene.add(d3.lights.dir)
    d3.lights.dir.castShadow = true

    const cursorGeometry = new THREE.PlaneGeometry(1, 1)
    const cursorMaterial = new THREE.MeshBasicMaterial({ color: 0x156289, side: THREE.DoubleSide })
    ...
    d3.materials.wood = new THREE.MeshLambertMaterial({ color: 0x9E8163, transparent: true })
    d3.materials.gray = new THREE.MeshLambertMaterial({ color: 0x4E4E4E, transparent: true })
    d3.materials.red = new THREE.MeshLambertMaterial({ color: 0x9E4E4E, transparent: true })
    d3.materials.green = new THREE.MeshLambertMaterial({ color: 0x4E9E4E, transparent: true })
  ...
  methods: {
    ...
    addPart(part) {
      const pm = ['rampup', 'rampdown'].includes(part)
        ? 'ramp' : part
      ...
      this.construction[mesh.uuid] = {
        part, position: [...this.cursor], rotation: 0,
        color: 'clear'
      }
      this.selection = mesh
    ...
    onClick(event) {
      ...
      if (intersects[0] && intersects[0].object) {
        if (this.selection && intersects[0].object.uuid === this.selection.uuid) {
          this.onDeselect()
        } else {
    ...
    setColor(color) {
      ...
      if (color === 'red')
        m = d3.materials.red
      if (color === 'green')
        m = d3.materials.green
      this.selection.material = m.clone()
      this.construction[this.selection.uuid].color = color
    },
    toggleFloor(l) {
      if (!this.hiddenFloors.includes(l))
        this.hiddenFloors.push(l)
      else
        this.hiddenFloors = this.hiddenFloors.filter(v => v !== l)
      this.setFloors()
    },
    setFloors() {
...
<template>
  <div class="home wh-100">
    <div class="view-3d wh-100 position-relative" ref="v3d">
      <canvas @click="onClick" ref="canvas" class="wh-100" @mousemove="onMove" />
      <UiPanel :construction="construction" :selected="selection ? selection.uuid : null" @remove="removeSelection"
        ...
        @save="save" @load-file="loadFile" @toggle-floor="toggleFloor" @update-pos="updateCursor" />
    </div>
  </div>
</template>
